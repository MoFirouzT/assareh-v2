...
- **Leakage probes** — honest primary;
  ...
  - *Statistical-discipline probes:*
    [D-004](DECISIONS.md#d-004--embargo-and-purging) embargo, [D-006](DECISIONS.md#d-006--barrier-touch-resolution-source) barrier-touch resolution, [D-010](DECISIONS.md#d-010--walk-forward-geometry) walk-forward geometry, [D-013](DECISIONS.md#d-013--feature-selection-scope) feature-selection scope.
  - *Data-handling probes* (surfaced by the v1 audit, [L-008](LEARNINGS.md#l-008--v1s-default-gap-interpolation-is-non-causal-and-contaminates-labels)–[L-011](LEARNINGS.md#l-011--v1-mixes-multi-tf-samples-by-counter-walking-not-by-timestamp-join)):
    [D-036](DECISIONS.md#d-036--gap-fill-discipline-leakage-probe) gap-fill discipline, [D-037](DECISIONS.md#d-037--feature-frame-nan-policy-leakage-probe) feature-frame NaN policy, [D-038](DECISIONS.md#d-038--patr-fill-policy-in-label-construction-leakage-probe) pATR fill policy, [D-039](DECISIONS.md#d-039--cross-timeframe-alignment-method-leakage-probe) cross-timeframe alignment method.
...
- **Retained comparison arms** —
  ...
  Members:
  [D-009](DECISIONS.md#d-009--loss-function) loss function (combined MSE+MAE vs. class-weighted BCE/focal), [D-011](DECISIONS.md#d-011--cost-model) cost model (gross v1-faithful vs. net honest, reported as gross/net metric pairs).
  These do not retire;
...
**Exit criteria:** label modules, splits, and held-out reservation tested; [D-008](DECISIONS.md#d-008--success-threshold-pre-registration) Stage 1 (threshold structure) appended; [D-014](DECISIONS.md#d-014--meta-labeling-side--size-decomposition) ratification queued for Phase E; **[D-040](DECISIONS.md#d-040--v1s-qualified-event-filter-consider_res) (qualified-event filter) resolved to Accepted/Rejected**; B-phase decision entries in DECISIONS.md.
...
- Triple-barrier target function with tests — **1m-resolved barrier ordering
  ...
  intra-bar tie-break by `close > open` direction ([D-028](DECISIONS.md#d-028--1m-intra-bar-tie-break-honest-arm)); **both barriers use
  ...
  ~48 bars (two-TF-higher rule, [D-003](DECISIONS.md#d-003--vertical-barrier-horizon-length-and-no-touch-handling); 511 reproduces v1)
- **Data-handling leakage probes wired into label construction**
  ([L-008](LEARNINGS.md#l-008--v1s-default-gap-interpolation-is-non-causal-and-contaminates-labels), [L-010](LEARNINGS.md#l-010--v1s-patr-series-is-ffillbfilld-inside-targetextractor23)). The v1-faithful labeling arm additionally reproduces
  *(i)* v1's non-causal `LinearInterpolator` gap fill on the 1m
  resolution substrate before barrier walking ([D-036](DECISIONS.md#d-036--gap-fill-discipline-leakage-probe)) and
  *(ii)* the `patr*.fillna('ffill').fillna('bfill')` chain inside
  `TargetExtractor` ([D-038](DECISIONS.md#d-038--patr-fill-policy-in-label-construction-leakage-probe)). The honest arm leaves gaps observed ([D-024](DECISIONS.md#d-024--gap-handling-soft-observation-never-forward-fill))
  and uses only pATR realised at or before `t`; bars where either is
  unresolvable emit typed-null labels ([D-029](DECISIONS.md#d-029--labelresult-schema-and-tail-sentinel-dtype)) rather than fabricated ones
- **Side label only (D-014 gate).** `make_labels` returns `rt3`, the three-class
  ...
  reproduced** — it was a discarded v1 experiment ([L-006](LEARNINGS.md#l-006--v1s-target2true-is-embedded-meta-labeling--but-it-was-a-failed-experiment) corrected, [L-017](LEARNINGS.md#l-017--reading-v1s-latest_code_and_results-notebooks-refines-does-not-overturn-several-docs)).
...
- Documented target statistics: positive rate, **timeout (no-touch) rate**,
  ...
  (cost-free) written down explicitly ([D-007](DECISIONS.md#d-007--breakeven-reference-385-not-50)); the cost-adjusted variant
  lands at the C/D handoff once [D-011](DECISIONS.md#d-011--cost-model)'s cost model is fixed, feeding the
...
- Walk-forward CV scheme implemented and tested, with **purging + embargo
  ...
  v1-faithful)** ([D-004](DECISIONS.md#d-004--embargo-and-purging)), concretely sized
  to **8 folds, ~2y initial anchor, ~1 quarter test per fold, ~6w val per
  fold** ([D-010](DECISIONS.md#d-010--walk-forward-geometry) added detail); the v1 single 75/15/10 chronological split
...
**Out of scope for this iteration:** CUSUM event filter ([D-015](DECISIONS.md#d-015--labeling-event-filter-sampling-cadence) — Rejected; revisit
...
**Exit criteria:** `evaluate()` runs all four baselines under every probe arm; per-arm parquet schema validated end-to-end on baseline output; [D-008](DECISIONS.md#d-008--success-threshold-pre-registration) Stage 2 (cost-adjusted breakeven value + `V` source) appended; [D-016](DECISIONS.md#d-016--backtest-geometry-walk-forward-vs-cpcv) CPCV verdict (implement or drop) recorded.
...
- Vectorized backtest function: takes signals + prices + cost assumptions,
  ...
  with the primary label arm, [D-006](DECISIONS.md#d-006--barrier-touch-resolution-source)); both **gross and net** P&L reported
  (gross lines up with v1, net is the honest number)
- Cost model ([D-011](DECISIONS.md#d-011--cost-model)): taker fee + slippage cushion sized to sub-1m touch
...
- **Per-arm metric record schema** (enables the Phase E headline finding).
  ...
  leakage-probe arms ([D-004](DECISIONS.md#d-004--embargo-and-purging) embargo, D-006 barrier resolution, [D-010](DECISIONS.md#d-010--walk-forward-geometry)
  geometry, [D-013](DECISIONS.md#d-013--feature-selection-scope) feature scope) share the honest arm's `metric_name` set,
...
- A reusable `evaluate(signals, prices, fold, *, arm) -> Metrics` interface;
  ...
  D-013 feature-selection scope; data-handling: [D-036](DECISIONS.md#d-036--gap-fill-discipline-leakage-probe) gap-fill, [D-037](DECISIONS.md#d-037--feature-frame-nan-policy-leakage-probe)
  NaN policy, [D-038](DECISIONS.md#d-038--patr-fill-policy-in-label-construction-leakage-probe) pATR fill, [D-039](DECISIONS.md#d-039--cross-timeframe-alignment-method-leakage-probe) cross-TF alignment) plus **2
  retained comparison arms** ([D-009](DECISIONS.md#d-009--loss-function) loss function, D-011 cost model
  gross/net). A single call site drives every arm — the runtime form of
  the [D-001](DECISIONS.md#d-001--dual-arm-methodology-governing-rule) governing rule
...
- Feature selection: Pearson correlation filter (0.95) + MI ranking as in v1,
  ...
  selection on 90% of data retained as a **selection-leakage probe** ([D-013](DECISIONS.md#d-013--feature-selection-scope))
- **Data-handling leakage probes wired into feature assembly**
  ([L-008](LEARNINGS.md#l-008--v1s-default-gap-interpolation-is-non-causal-and-contaminates-labels), [L-009](LEARNINGS.md#l-009--v1s-datamixerload_features-applies-blanket-bfill-to-every-feature-column), [L-011](LEARNINGS.md#l-011--v1-mixes-multi-tf-samples-by-counter-walking-not-by-timestamp-join)). The v1-faithful feature arm additionally
  reproduces *(i)* v1's non-causal `LinearInterpolator` gap fill on
  every input timeframe before indicator compute ([D-036](DECISIONS.md#d-036--gap-fill-discipline-leakage-probe), downstream
  side), *(ii)* the blanket `DataMixer.*.load_features` `bfill` on the
  assembled feature frame ([D-037](DECISIONS.md#d-037--feature-frame-nan-policy-leakage-probe)), and *(iii)* the
  `DataMixer3._mix_train` counter-walked multi-TF mix ([D-039](DECISIONS.md#d-039--cross-timeframe-alignment-method-leakage-probe)) in place of
...
**Exit criteria:** [D-014](DECISIONS.md#d-014--meta-labeling-side--size-decomposition) verdict appended; gap artifact written to `reports/gap.parquet`; success-threshold check executed and the verdict (pass / null) recorded; aggregate per-arm metrics logged to MLflow.
...
- **Ratified.** Add a binary meta-model for bars where the primary takes a
  ...
  produced; [L-006](LEARNINGS.md#l-006--v1s-target2true-is-embedded-meta-labeling--but-it-was-a-failed-experiment) corrected, [L-017](LEARNINGS.md#l-017--reading-v1s-latest_code_and_results-notebooks-refines-does-not-overturn-several-docs)). Action threshold = cost-adjusted breakeven
  ([D-007](DECISIONS.md#d-007--breakeven-reference-385-not-50)); sizing by conviction above breakeven (de Prado's probability→size map,
...
- **Deferred (follow-on iteration).** No meta-model this iteration. Phase E's
  ...
  "no-trade") carries the iteration. [D-009](DECISIONS.md#d-009--loss-function)'s loss arms remain the dual-arm.

The verdict is appended to D-014 (Proposed → Accepted or Superseded) and to [D-008](DECISIONS.md#d-008--success-threshold-pre-registration)'s Stage 2 commit, before any model is fit.
...
- **The honest-vs-v1-faithful gap artifact** — the project's headline
  ...
  - *Statistical-discipline probes:* [D-004](DECISIONS.md#d-004--embargo-and-purging) embargo, [D-006](DECISIONS.md#d-006--barrier-touch-resolution-source) barrier
    resolution, [D-010](DECISIONS.md#d-010--walk-forward-geometry) walk-forward geometry, [D-013](DECISIONS.md#d-013--feature-selection-scope) feature-selection
    scope.
  - *Data-handling probes:* [D-036](DECISIONS.md#d-036--gap-fill-discipline-leakage-probe) gap-fill discipline, [D-037](DECISIONS.md#d-037--feature-frame-nan-policy-leakage-probe)
    feature-frame NaN policy, [D-038](DECISIONS.md#d-038--patr-fill-policy-in-label-construction-leakage-probe) pATR fill policy, [D-039](DECISIONS.md#d-039--cross-timeframe-alignment-method-leakage-probe)
